...
function getMoreArticles() {
    ...
    .then(function (data) {
        data.map(element => {
            if (element['Message']) {
                let btnGetArticles = document.getElementById('get-articles')
                btnGetArticles.style.display = 'none'
            } else {
                ...
                <article id="${element.id}" class="article last:mb-10 my-24 relative">
                    ...
                    <p class='paragraph text-xl sm:text-base leading-8 font-light my-5 font-["Roboto"] text-slate-800 dark:text-slate-300 whitespace-pre-wrap'>${element.summary}</p>
                </article>
                `
            }
        })
